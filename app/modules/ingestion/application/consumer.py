...
def flush_batch(channel):
    ...
    if _batch_orms:
        ...
        try:
            ...
            # Upsert trade records for swap/transfer events (non-blocking)
            try:
                ...
                try:
                    for orm in _batch_orms:
                        ...
                        result = upsert_trade_record_from_event(trade_db, event_dict)
                        if result in ("created", "updated"):
                            trade_count += 1
                    trade_db.commit()
                    if trade_count > 0:
                        logger.info(f"[BATCH] ✅ Trade records upserted: {trade_count}")
