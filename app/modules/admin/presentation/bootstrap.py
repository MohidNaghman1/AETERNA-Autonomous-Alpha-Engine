...
def create_first_admin(data: CreateFirstAdminRequest):
    ...
    **BOOTSTRAP ONLY** - Only works if no admin exists yet!
    ...
    try:
        ...
        existing_admin = db.query(UserRole).filter(UserRole.role == "admin").first()

        if existing_admin:
            logger.warning(
                f"Bootstrap attempt when admin exists: user {existing_admin.user_id}"
            )
            raise HTTPException(
        ...
        existing_role = (
            db.query(UserRole).filter(UserRole.user_id == data.user_id).first()
        )
