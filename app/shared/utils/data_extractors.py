...
def extract_rss_entry_detailed(entry: Dict[str, Any], source: str) -> Dict[str, Any]:
    ...
    hashtags = extract_hashtags(title + " " + summary) if summary else extract_hashtags(title)
    hashtags = list(set(hashtags))[:10]  # Limit to top 10
    
    mentions = extract_mentions(title + " " + summary) if summary else extract_mentions(title)
    mentions = list(set(mentions))[:10]  # Limit to top 10
...
def extract_price_entry_detailed(entry: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if high_24h and low_24h and current_price:
        price_position_24h = (current_price - low_24h) / (high_24h - low_24h) if high_24h != low_24h else 0.5
    ...
    if trading_volume_24h and market_cap:
        volume_to_market_cap = round(trading_volume_24h / market_cap, 4) if market_cap != 0 else None
...
def calculate_crypto_risk_score(entry: Dict[str, Any], price_changes: Dict[str, Any]) -> float:
    """
