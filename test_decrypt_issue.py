...
def trigger_rss_collection():
    ...
    try:
        status, response = make_request(
            f"{BASE_URL}/ingestion/trigger-rss-collection", method="POST", timeout=30
        )
        if status == 200:
...
def poll_consumer(batch_size=1000):
    ...
    try:
        ...
        if status == 200:
            ...
            print(
                f"✅ Consumer polled: processed {count} messages (batch_size={actual_batch})"
            )
            return count
        else:
...
def check_rss_collection_details():
    ...
    try:
        status, response = make_request(
            f"{BASE_URL}/ingestion/diagnostic/test-rss-sync", timeout=120
        )
        if status == 200:
            data = json.loads(response)
            if "results" in data:
...
def check_queue_depth():
    ...
    try:
        status, response = make_request(
            f"{BASE_URL}/ingestion/diagnostic/rabbitmq-queue-depth", timeout=30
        )
        if status == 200:
            data = json.loads(response)
...
if __name__ == "__main__":
    ...
    if rss_detail:
        ...
        print(
            f"    Normalization errors: {rss_detail.get('normalization_errors', 'N/A')}"
        )
    else:
    ...
    if queue_after is not None:
        ...
        print(
            f"    Messages consumed by this poll: {queue_before - queue_after if queue_before else 'unknown'}"
        )
    else:
        print("    ⚠️ Could not check queue depth")
    ...
    if decrypt_added > 0:
        ...
        print(
            f"⚠️  PARTIAL: Consumer processed {processed} messages but Decrypt didn't increase"
        )
